...
def _highlight_menu_command(suggest: SlashSuggest, command: str) -> None:
    """Set up a submission test without exercising unrelated arrow navigation."""
    for index in range(suggest.option_count):
        suggest.highlighted = index
        spec = suggest.highlighted_spec()
        if spec is not None and spec.command == command:
            return
    raise AssertionError(f"no menu option for {command!r}")


def test_textual_enter_runs_highlighted_argless_command_without_typing_it() -> None:
    # Type only "/", highlight an arg-less command, press Enter -> that
    ...
    async def scenario() -> tuple[str, bool]:
        ...
        async with app_instance.run_test() as pilot:
            ...
            await pilot.pause()
            _highlight_menu_command(suggest, "/quit")
            await pilot.press("enter")
            await pilot.pause()
            queued = await app_instance._input_controller.receive_stream.receive()
            return queued, suggest.is_open
...
def test_textual_enter_executes_highlighted_optional_arg_command() -> None:
    ...
    async def scenario() -> tuple[str, str]:
        ...
        async with app_instance.run_test() as pilot:
            ...
            await pilot.pause()
            _highlight_menu_command(suggest, "/auth")
            await pilot.press("enter")
            await pilot.pause()
            queued = await app_instance._input_controller.receive_stream.receive()
...
def test_textual_disconnect_selection_opens_picker_command() -> None:
    async def scenario() -> tuple[str, str]:
        ...
        async with app_instance.run_test() as pilot:
            ...
            await pilot.press("/")
            await pilot.pause()
            _highlight_menu_command(suggest, "/disconnect")
            await pilot.press("enter")
            await pilot.pause()
            submitted = await app_instance._input_controller.receive_stream.receive()
...
def test_textual_menu_arrow_navigation_wraps_without_taking_editor_focus() -> None:
    async def scenario() -> None:
        app_instance = TextualTui()
        async with app_instance.run_test() as pilot:
            editor = app_instance.query_one("#input", Input)
            ...
            await pilot.press("/")
            await pilot.pause()
            assert suggest.highlighted == 0
            await pilot.press("up")
            assert suggest.highlighted == suggest.option_count - 1
            await pilot.press("down")
            assert suggest.highlighted == 0
            await pilot.press("down")
            assert suggest.highlighted == 1
            assert app_instance.focused is editor
            spec = suggest.highlighted_spec()
            assert spec is not None
            await pilot.press("enter")
            submitted = await app_instance._input_controller.receive_stream.receive()
            assert submitted == spec.command

...
def test_session_writes_versioned_discriminated_entries(tmp_path: Path) -> None:
    ...
    assert records[3]["event"]["payload"]["schema_version"] == 13
...
def test_session_retains_future_event_payload_until_typed_access(tmp_path: Path) -> None:
    ...
    raw_event = {"type": "future.event", "schema_version": 14, "future": True}
    legacy = {
        "id": "future-event",
        "session_id": "event-session",
        "kind": "event",
    ...
    assert session.read_events() == (raw_event,)
    with pytest.raises(UnsupportedPersistedEventVersionError, match="schema_version 14"):

...
_FINAL_SUMMARY = (
    ...
    "tests/agent/loop/test_runner.py` passed with exit code 0; the longer `uv run pytest` "
...
def test_workflow_handles_branch_timeout_and_completion(
    ...
    focused_test = _bash_call(
        "focused-test",
        "uv run pytest tests/agent/loop/test_runner.py",
        timeout=30,
    )
    ...
    outcomes = {
        ...
        focused_test.call_id: _BashOutcome(
            command="uv run pytest tests/agent/loop/test_runner.py",
            timeout=30,
            output="Command exited with code 0\n37 passed",
            is_error=False,
            exit_code=0,

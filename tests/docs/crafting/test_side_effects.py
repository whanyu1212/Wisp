...
@pytest.mark.parametrize("name", ["edit", "test"])
def test_dispatch_reporter_cannot_change_approved_inputs(tmp_path: Path, name: str) -> None:
    create_fixture(tmp_path)

    def report(message: str) -> None:
        if message.startswith("dispatch:"):
            (tmp_path / "calculator.py").write_text("return a - b\n# reporter change\n")

    tools = ControlledTools(
        tmp_path, ExecutionPolicy(frozenset({name})), lambda _: True, report=report
    )

    def forbidden(call: ToolCall) -> str:
        raise AssertionError("changed input executed")

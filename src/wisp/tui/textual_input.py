...
class TextualInputController:
    ...
    @property
    def receive_stream(self) -> MemoryObjectReceiveStream[TuiSubmission | BaseException]:
    ...
    def set_submit_hook(self, on_submit: Callable[[], str | None]) -> None:
        """Set the callback that captures the mode of each accepted line."""

        self._on_submit = on_submit

    async def receive(self) -> TuiSubmission:
    ...
    def submit_line(
        ...
        *,
        ...
        input_mode = self._on_submit() if self._on_submit is not None else None
        ...
        try:
            ...
            return False
        if clear_editor:
            buffer_submission = getattr(self._surface, "buffer_submission", None)
            if callable(buffer_submission):
                buffer_submission(submission)

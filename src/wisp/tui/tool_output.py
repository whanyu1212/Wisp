...
* Edit/write card rendering returns a structured ``DiffPresentation`` whose rows
  ...
  return ``Content`` for focused renderer tests. File content is never parsed as
  markup, so a diff line containing ``[red]`` or ``[/]`` cannot inject or break a
  color span.
...
def render_tool_result(
    ...
    *,
    ...
    if name == "edit":
        diff = build_edit_diff_presentation(arguments)
        if diff is not None:
            return diff
    elif name == "write":
...
def render_edit_diff(arguments: Mapping[str, object]) -> Content | None:
    """Render an ``edit`` tool call's hunks as the legacy bounded unified diff.

    The public helper remains a ``Content`` renderer for the existing focused
    diff tests and non-card callers. Tool cards use
    :func:`build_edit_diff_presentation`, which retains structured rows for
    width-aware and expandable display without reparsing rendered text.
    """

    spec = _edit_diff_spec(arguments)
    if spec is None:
        return None
    changed, multi, path = spec
    return _render_diff_content(changed, multi=multi, path=path)


def build_edit_diff_presentation(
    arguments: Mapping[str, object],
) -> DiffPresentation | None:
    """Build a structured diff for a successful ``edit`` result."""
...
def render_write_diff(
    ...
    *,
    ...
    """Render a ``write`` tool call as the legacy bounded unified diff."""

    spec = _write_diff_spec(before, arguments, created=created)
    if spec is None:
        return None
    changed, path, _operation = spec
    return _render_diff_content(changed, multi=False, path=path)


def build_write_diff_presentation(
    before: str | None,
    arguments: Mapping[str, object],
    *,
    created: bool = False,
) -> DiffPresentation | None:
    """Build a structured diff for a successful ``write`` result."""
...
def _edit_diff_spec(
    arguments: Mapping[str, object],
) -> tuple[list[tuple[int, str, str]], bool, object] | None:
    """Validate, normalize, and select changed ``edit`` hunks once."""
    ...
    if not changed:
        return None
    return changed, multi, arguments.get("path")


def _write_diff_spec(
    before: str | None,
    arguments: Mapping[str, object],
    *,
    created: bool,
) -> tuple[list[tuple[int, str, str]], object, DiffOperation] | None:
    """Validate and normalize one ``write`` before/after pair."""

    after = arguments.get("content")
    if not isinstance(after, str) or (before is None and not created):
        return None
    ...
    return (
        [(1, old, new)],
        arguments.get("path"),
        (DiffOperation.create if created else DiffOperation.modify),
    )
...
def _render_diff_content(
    ...
    *,
    ...
    """Build the established bounded ``Content`` view from validated hunks."""

...
command, and requires Python 3.12 or newer. The `0.1.0rc5` release candidate supports Linux and
macOS; Windows remains best-effort until it has dedicated CI coverage.

Request the prerelease explicitly:

```bash
uv tool install "wisp-ai==0.1.0rc5"
```
...
```bash
uvx --from "wisp-ai==0.1.0rc5" wisp
```

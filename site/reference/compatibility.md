...
emit an event schema much newer than v37.
...
`schemas/live-rpc/v6/`. Ordinary command envelopes inherit the selected connection version rather
than carrying their own version. Events continue to carry the independently negotiated
`schema_version` described below.

The v6 schema bundle contains handshake request and response messages, the complete typed-client
...
[bounded project file discovery](./project-files). All earlier bundles remain immutable.
...
emits only `EVENT_SCHEMA_VERSION`, currently **v37**. There is no transport-level negotiation or
supported down-level emission mode.

The typed parsers currently read **v5 through v37**:
...
[event-schema history](https://github.com/whanyu1212/Wisp/blob/main/CHANGELOG.md#schema-v37--current)
...
- consult the [event-schema history](https://github.com/whanyu1212/Wisp/blob/main/CHANGELOG.md#schema-v37--current)
...
| Event payload inside the envelope | v37 | v5–v37 for typed access |

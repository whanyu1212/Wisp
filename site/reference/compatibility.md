...
emit an event schema much newer than v38.
...
`schemas/live-rpc/v7/`. Ordinary command envelopes inherit the selected connection version rather
than carrying their own version. Events continue to carry the independently negotiated
`schema_version` described below.

The v7 schema bundle contains handshake request and response messages, the complete typed-client
...
and the explicit `all_project` approval scope; `all_session` remains temporary. All earlier bundles
remain immutable.
...
emits only `EVENT_SCHEMA_VERSION`, currently **v38**. There is no transport-level negotiation or
supported down-level emission mode.

The typed parsers currently read **v5 through v38**:
...
[event-schema history](https://github.com/whanyu1212/Wisp/blob/main/CHANGELOG.md#schema-v38--current)
...
- consult the [event-schema history](https://github.com/whanyu1212/Wisp/blob/main/CHANGELOG.md#schema-v38--current)
...
| Event payload inside the envelope | v38 | v5–v38 for typed access |
